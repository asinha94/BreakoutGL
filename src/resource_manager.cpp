...
namespace Bout {
    ...
    {
        auto vShader = VertexShader(vShaderFile);
        auto fShader = FragmentShader(fShaderFile);

        // 2. now create shader object from source code
        auto shader = gShaderFile != nullptr ?
            ShaderProgram(vShader, fShader, GeometryShader(gShaderFile)) : ShaderProgram(vShader, fShader);
        return shader;
    }
